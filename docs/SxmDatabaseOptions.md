# SxmDatabaseOptions Configuration Guide
...
Initialize your database with custom options:

```csharp
using SQLiteXM;

// Create options with desired settings
...
**When to use:**
- ✅ Enable when you need referential integrity
- ❌ Disable for legacy databases or bulk operations where you manage integrity manually
...
var options = new SxmDatabaseOptions
{
	...
	CheckPointWalMaxSize = 32  // Checkpoint when WAL > 32 KB
};
```
...
{
	CheckPointConnection = CheckPointConnection.MaxSize,
	CheckPointWalMaxSize = 32  // 32 KB limit
};
```

**Recommendations:**
- **Mobile apps:** 32 KB - 128 KB (keep storage usage low)
- **Desktop apps:** 256 KB - 1 MB
- **Server apps:** 1 MB - 10 MB
...
- [SxmDatabase Initialization](./SxmDatabase.md)
