...
public class MultiDatabasePerformanceTests : IDisposable
{
  ...
  ""databases"": [
    ...
    public async Task BulkInsert_AcrossMultipleDatabases_PerformsWell()
    {
        ...
        await using (var productTransaction = await SxmTransaction.CreateAsync(productConnection))
        {
        ...
        await using (var orderTransaction = await SxmTransaction.CreateAsync(orderConnection))
        {
            for (int i = 0; i < 5_000; i++)
            {
        ...
        await using (var auditTransaction = await SxmTransaction.CreateAsync(auditConnection))
        {
            for (int i = 0; i < 5_000; i++)
            {
        ...
        // Verify counts
        using (var productContext = new SxmTransaction("products"))
        {
            Assert.Equal(5_000, productContext.GetTable<Product>().Count());
        }
        using (var orderContext = new SxmTransaction("orders"))
        {
            Assert.Equal(5_000, orderContext.GetTable<Order>().Count());
        }
        using (var auditContext = new SxmTransaction("audit"))
        {
    ...
    public async Task Aggregates_LargeDataset_PerformEfficiently()
    {
        ...
        await using (var context = new SxmTransaction("orders"))
        {
